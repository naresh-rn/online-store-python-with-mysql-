...
# this for atomatically generate
r = random.randint(1111, 9999)
user_id = "%04d" % r                            # random for - USER_ID
o_id = random.randint(555555,999999)
ord_id = "%06d" % o_id                          # random for - ORDER_ID
e_id = random.randint(22111,50909)
emp_code = "%05d" % e_id                        # random for - EMPLOYEE_ID
p_code = random.randint(1000,3000)
proid = "%4d" % p_code                          # random for - PRODUCT_ID
...
# function for Geting user List form the Mysql Database
def get_user():
...
# Function for Getting for order details
def get_order():
...
# function for Getting the Entire Products from Mysql to py IDE
...
# this function for getting Login details
def user_login():
...
if option == "A":
    ...
    if user == 1:
        ...
        print(f"{user} invalid option. please check!")
else:
